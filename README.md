...
2. 在 Notion 裡建立一個 **Students 索引資料庫**，欄位如下：
   ...
   | `PIN` | Rich text | 選填。點名時要輸入這組 PIN 才能開始玩。**注意：這只是防誤觸的小提醒，不是真正的安全機制**——PIN 會跟其他資料一起被打包進公開網站的原始碼，任何人打開瀏覽器「檢視原始碼」都看得到，防不了真的想看的人。留空就不用輸入 PIN，直接點名字進去 |
...
- 點名清單、每個學生的姓名、以及選填的 PIN 碼，都是**公開網站原始碼的一部分**，任何知道網址的人都看得到（PIN 只防誤觸，不是真安全）。
